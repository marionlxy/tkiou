package com.taikang.udp.threadpool.action.impl;
...
import com.taikang.udp.threadpool.action.ITestThreadAction;
 ...
 */
@Scope("prototype")
@Service(ITestThreadAction.ACTION_ID)
public class TestThreadActionImpl extends BaseActionImpl implements ITestThreadAction,
		ITkTaskService {
	...
	public void updateObject(Dto param) {
		...
		if (param.getAsString("comParent") != null
			...
			delcomRelaChild.setChildId(Integer.valueOf(param
					.getAsString("comId")));
			comRelaService.deleteObjectByChildId(delcomRelaChild);
			ComRelaBO delcomRela = new ComRelaBO();
			delcomRela.setParentId(Integer.valueOf(param
					.getAsString("comParent")));
	...
	public void execute(Dto dto) {
		List<Dto> list = comService.findAllMap(dto);
		logger.debug("------------");
		logger.debug("------------");
