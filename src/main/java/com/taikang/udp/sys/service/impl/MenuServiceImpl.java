 ...
 implements IMenuService<MenuBO>  
  {
	...
	*/
	...
	public List<MenuTree> getMenuTree(String menuId, String state,
		...
		for (int i = 0; i < menuLst.size(); i++) {
			...
			tree.setUrl(childMenuBO.getMenuUrl() + "?reqMenuId=" + childMenuBO.getMenuId()
					/*"&menuName=" +childMenuBO.getMenuName()*/+"&parentId="+childMenuBO.getParentId()+"&backUp2="+childMenuBO.getBackup2()+"&isLeaf="+childMenuBO.getIsLeaf()+"&status="+childMenuBO.getMenuStatus());
			tree.setState(state);
			//
	...
	/* (non-Javadoc)
	 * @see com.taikang.udp.sys.service.IMenuService#updateMenuSubObject(com.taikang.udp.sys.model.MenuBO)
	 */
	@Override
	public void updateMenuSubObject(MenuBO menuBO) {
		// TODO Auto-generated method stub
		logger.debug("<======MenuServiceImpl--updateMenu======>");
		if(!menuBO.getBackup2().isEmpty()){
			appDao.update("Menu.updateSubMenu",menuBO);
		}
		if(!menuBO.getParentId().isEmpty()){
			appDao.update("Menu.updateParenTSubMenu",menuBO);
		}
	}
 }
